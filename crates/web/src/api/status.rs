...
async fn get_status(
    ...
    // Read status from the WEB DB (state.db), NOT the sync engine's DB.
    // The sync engine's DB mutex is held by the scheduler thread during
    // sync cycles. Using the web DB avoids all contention.
    let db = &state.db;

    let state_str = db.get_state("sync_state")
        .unwrap_or(None)
    ...
    let last_svn_rev = db.get_state("last_svn_rev")
        .unwrap_or(None)
        .and_then(|s| s.parse::<i64>().ok())
        .or_else(|| db.get_last_svn_revision().ok().flatten());
    let last_git_hash = db.get_state("last_git_hash")
        .unwrap_or(None)
    ...
    let status = gitsvnsync_core::models::SyncStatus {
        state: gitsvnsync_core::models::SyncState::from_str_val(&state_str),
        last_sync_at,
